...
# %%
#Change Directory
os.chdir(r'C:\Users\prans\Face-Recognition-and-Attendance-monitoring-system')
os.listdir()  #path where file is
...
while True:
    ...
    for encodeface,faceloc in zip(encodecurframe,faceincurframe):
        ...
        if matches[matchindex]:
            ...
            cv2.rectangle(img, (x1,y1), (x2,y2), (0,0,255) ,2)   #Hollow rectangle around the face
            cv2.rectangle(img , (x1,y2-30), (x2,y2), (0,255,255) ,cv2.FILLED)  #Filled rectangle below hollow rectangle to print name on it
            cv2.putText(img,name,(x1+4,y2-6),cv2.FONT_HERSHEY_PLAIN,1,(0,0,0),2)    
            attendance(name)

    cv2.imshow("Webcam" , img)
    #to close camera press ENTER
    if cv2.waitKey(1)==13:
        break
